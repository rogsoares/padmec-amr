...
Remover::~Remover (){set<pEdge> PlaneSurfaceEdges;
...
CommonVertex.clear();
triangles.clear();
VertexClsAux.clear(); // Colocar esse como uma variavel local da funcao...
VertexCls.clear();
...
void Remover::SaveBGMView1(double Cl,float GE){

	MtxTriangles = new eleMatriz[triangles.size()];


	/*
	system("rm TempView2D.pos");  //OBSERVAÇÃO IMPORTANTE, o comando rm só funciona no linux

	cout << endl;
	cout << "Salvando View 2D - TempView2D.pos" << endl;
	ofstream Myfile("./TempView2D.pos", ios::app);


	if (!Myfile) {
		Myfile.open("./TempView2D.pos");
	}
	...
	//std::map<int,float>::iterator ITF;
	...
	for(itFaces=triangles.begin(), c=0; (itFaces!=triangles.end()) && (triangles.size() > c) ; c++,itFaces++){ // ANALISAR ESTE LACO, VERIFICAR SE O TRIANGULOS ESTAO SENDO TODOS COLOCADOS CORRETAMENTE NA MATRIZ - LIMITES INFERIOR E SUPERIOR.
		...
		double height;
		EN_getDataDbl(Point1,MD_lookupMeshDataId( "elem_height" ),&height);
		// Rotina para encontrar no map e atribuir o cl do ponto
		//	if(BoundaryNodes.count(Point1)==0){
		//MtxTriangles[c].Cl1 = VertexCls.find(EN_id(Point1))->second;
		MtxTriangles[c].Cl1 = (float)height;

		//	}
		//	else{
		//		MtxTriangles[c].Cl1=GE;
		//	}
		EN_getDataDbl(Point2,MD_lookupMeshDataId( "elem_height" ),&height);
		//	if(BoundaryNodes.count(Point2)==0){
		//MtxTriangles[c].Cl2 = VertexCls.find(EN_id(Point2))->second;
		MtxTriangles[c].Cl2 = (float)height;
		//	}
		//	else{
		//		MtxTriangles[c].Cl2=GE;
		//	}

		//	if(BoundaryNodes.count(Point3)==0){
		//MtxTriangles[c].Cl3 = VertexCls.find(EN_id(Point3))->second;
		EN_getDataDbl(Point3,MD_lookupMeshDataId( "elem_height" ),&height);
		MtxTriangles[c].Cl3 = (float)height;
		//	}
		//	else{
		//		MtxTriangles[c].Cl3=GE;
		//	}
		...
		MtxTriangles[c].x1=xyz1[0];
		MtxTriangles[c].y1=xyz1[1];
		MtxTriangles[c].z1=xyz1[2];

		MtxTriangles[c].x2=xyz2[0];
		MtxTriangles[c].y2=xyz2[1];
		MtxTriangles[c].z2=xyz2[2];

		MtxTriangles[c].x3=xyz3[0];
		MtxTriangles[c].y3=xyz3[1];
		MtxTriangles[c].z3=xyz3[2];

		MtxTriangles[c].Pt1=Point1;
		MtxTriangles[c].Pt2=Point2;
		MtxTriangles[c].Pt3=Point3;
...
void Remover::SaveBGMView2(float GE) {
	...
	for ( itnodes=BoundaryNodes.begin() ; itnodes != BoundaryNodes.end(); itnodes++ ){
		...
		//SP(-0.5,-0.6,0){0.1};
		Myfile<<"SP("<< xyz[0] <<","<<" "<<xyz[1]<<","<<" "<<xyz[2]<<")"<<"{"<<GE<<"};"<<endl;
	...
	for(int c=0;c<triangles.size();c++){
		...
		Myfile << "ST(" << MtxTriangles[c].x1 << ", " << MtxTriangles[c].y1 << ", " << MtxTriangles[c].z1 << ", " << MtxTriangles[c].x2 << ", " << MtxTriangles[c].y2 << ", " << MtxTriangles[c].z2 << ", " << MtxTriangles[c].x3 << ", " << MtxTriangles[c].y3 << ", " << MtxTriangles[c].z3 << "){" << MtxTriangles[c].Cl1 << ", " << MtxTriangles[c].Cl2 << ", " << MtxTriangles[c].Cl3 << "};" << endl;
...
void Remover::ClReader(char *CLS){ // Esta funcao le o arquivo de lista de cls, onde a primeira parte indica os ids dos pontos e a segunda parte indica os cls desses pontos, cria um map para ser usado pela funcao que gera o arquivo .pos da malha de background
	ifstream infile;
	infile.open (CLS, ifstream::in);
	...
	while (EndofNodes == 0 ){
		getline (infile,line);
		if (!line.empty()){
			i = strncmp(line.c_str(), Limit.c_str(),6);
			if ( i == 0 ){
				EndofNodes=1;
			}
			else{
				contagem++;
				Lineflo = atof(line.c_str());
				VertexClsAux.insert(std::pair<int,float>(contagem,Lineflo));
			}
		}
	}
	...
	while (! infile.eof() ){
		...
		getline (infile,line);
		if (!line.empty()){
			itf=VertexClsAux.find(contagem);
			VertexCls.insert(std::pair<int,float>(atoi(line.c_str()),itf->second) );
		}
	}

	for ( itf=VertexCls.begin(); itf!=VertexCls.end(); ++itf){
...
int Remover::Iterador(set <int> ListofElements, int I, pMesh theMesh){ // Esta funcao percorre toda a malha uma vez e levanta as informacoes necessárias

	int GreaterID=0;
	cout << "Entrou no iterador" << endl;
	...
	if (I==2){
		...
		for ( itElements=ListofElements.begin() ; itElements != ListofElements.end(); itElements++ ){
			...
			while (pFace Fc = FIter_next(fit)){
				if (EN_id(Fc)==*itElements){

					BoundaryFaces.insert(Fc);
					triangles.insert(Fc);
...
void Remover::BoundaryElements2D(set <int> ListofElements, double Cl, int VerCL,pMesh theMesh){
	...
	for ( itedges=BoundaryEdges.begin() ; itedges != BoundaryEdges.end(); itedges++ ){
		pEdge Edge = *itedges;
		pVertex vertex1 = E_vertex(Edge,0);
		pVertex vertex2 = E_vertex(Edge,1);
		...
		float EdgeLength = sqrt ((xyz1[0]-xyz2[0])*(xyz1[0]-xyz2[0]) + (xyz1[1]-xyz2[1])*(xyz1[1]-xyz2[1]) + (xyz1[2]-xyz2[2])*(xyz1[2]-xyz2[2]) );

		if (EdgeLength>GreatestEdge){
			GreatestEdge = EdgeLength;
		}
	...
	if (VerCL==0){
		for ( itvertex=BoundaryNodes.begin() ; itvertex != BoundaryNodes.end(); itvertex++ ){
			EN_attachDataDbl (*itvertex, MD_lookupMeshDataId("CL"), GreatestEdge); // Coloquei GreatestEdge no lugar de Cl pq quero que ela seja o Cl do contorno dos elementos
		}
		VerCL=1;
	}
}
...
void Remover::removeinternalfaces(pMesh theMesh){
	...
	for ( itfaces=BoundaryFaces.begin() ; itfaces != BoundaryFaces.end(); itfaces++ ){
		...
		if (F_numRegions(*itfaces)==0){

			if (GeomBoundFaces.find(*itfaces)==GeomBoundFaces.end()){ // NAO PRECISA SER SET
				theMesh->DEL(*itfaces);
				RemovefromBoundFaces.insert(*itfaces);
			}
		}
	}
	for ( itfaces=RemovefromBoundFaces.begin() ; itfaces != RemovefromBoundFaces.end(); itfaces++ ){
		BoundaryFaces.erase(*itfaces);
	}
