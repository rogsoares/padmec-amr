...
FileManager::~FileManager (){
	BoundVertex.clear(); // mapa dos vertices da malha original necessário para merge_msh encontrar os BoundVertex e comparar com os pontos que ela está criando.
...
void FileManager::SaveFileGeometry_2D(float Cl, pMesh theMesh, int PSCounter){ // Salva um ARQUIVO DE GEOMETRIA padrao do gmesh DO CONTORNO DO BURACO para gerar a malha com o cl escolhido   
	...
	while (P<=PSCounter){
		...
		while (w==1){
			...
			F->CreateLineLoop(CommonEdges, c,"Geometria2D",theMesh);  //matriz1 criada passa-la para a rotina de reordenacão para depois criar a curva. CreateLineLoop reordena estes edges como um lineloop - LL é um int ( o id do lineloop)
