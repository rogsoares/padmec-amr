...
namespace PRS{
	...
	EBFV1_hyperbolic_MIMPES_Adapt::EBFV1_hyperbolic_MIMPES_Adapt(){
	}

	EBFV1_hyperbolic_MIMPES_Adapt::EBFV1_hyperbolic_MIMPES_Adapt(pMesh mesh,PhysicPropData *ppd, SimulatorParameters *sp,GeomData *gcd,MeshData *md, OilProductionManagement *popm, ErrorAnalysis *pEAna){
		pMData = md;
		pOPManager = popm;
		pPPData = ppd;
		pGCData = gcd;
	...
	double EBFV1_hyperbolic_MIMPES_Adapt::solver(pMesh theMesh, double &timeStep){
		...
		do{     // keep velocity field constant if possible
			...
			// evaluate saturation error field and check if tolerance is still satisfied
			// if error is higher than tolerance, stop MIMPES and go to adaptation
			if (1){//(calculate_ErrorAnalysis(pEA,pSimPar,pGCData,PhysicPropData::getGradient)){
				go_MIMPES = false;
			}
			else{
				timestep_counter++;
	...
	void EBFV1_hyperbolic_MIMPES_Adapt::calculateImplicitTS(double &p_timestep, double timeStep, double &DV_norm, bool &go_ImplicitTS, bool &go_MIMPES){
		if (go_ImplicitTS){
			CPU_Profile::Start();
	...
	void EBFV1_hyperbolic_MIMPES_Adapt::correct_p_TS(double &p_timestep, bool &go_MIMPES){
	...
	void EBFV1_hyperbolic_MIMPES_Adapt::correct_Sw_TS(double &timeStep, bool &go_MIMPES){
	...
	void EBFV1_hyperbolic_MIMPES_Adapt::calculateVelocityVariationNorm(double &DV_norm, int dim){
		CPU_Profile::Start();
	...
	void EBFV1_hyperbolic_MIMPES_Adapt::MIMPES_output(double Sw_timestep_sum, double p_timestep, int numCFL_Steps, int sumNum_p_DT, int sumNum_Sw_DT, double DV_norm){
	...
	void EBFV1_hyperbolic_MIMPES_Adapt::createDimensionLessFactors(){
		double L, H, phi;
