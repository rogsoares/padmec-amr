...
//		if ( pSimPar->userRequiresAdaptation() ){
			...
			std::list<pEntity> elemToAdapt;
			std::set<pEntity> backgroundNodes;
			calculate_ErrorAnalysis(pErrorAnalysis,theMesh,pSimPar,pGCData,pPPData->get_FuncPointer_GetGradient(),elemToAdapt,backgroundNodes);
//			if (adapt){
